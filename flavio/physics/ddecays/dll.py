...
def amplitudes(par, wc, l1, l2):
    r"""Amplitudes P and S entering the $D_q\to\ell_1^+\ell_2^-$ observables.
...
def br_inst(par, wc,  l1, l2):
    ...
    N = ckm.xi('b','cu')(par) * 4*GF/sqrt(2) * alphaem/(4*pi)
...
for l in ['e', 'mu', 'tau']:
    ...
    _process_tex = r"D_0\to "+_tex[l]+r"^+"+_tex[l]+r"^-"
    ...
    _obs.tex = r"$\tau_{D_0 \to " +_tex[l] +_tex[l] + "}$"
    _obs.add_taxonomy(_process_taxonomy + _process_tex + r"$")
    if l=='e':
        Prediction(_obs_name, lambda wc_obj, par: tau_ll_func(wc_obj, par, 'D0', 'e'))
    if l=='mu':
        Prediction(_obs_name, lambda wc_obj, par: tau_ll_func(wc_obj, par, 'D0', 'mu'))
    if l=='tau':
        Prediction(_obs_name, lambda wc_obj, par: tau_ll_func(wc_obj, par, 'D0', 'tau'))

    _obs_name = 'ADeltaGamma(D0->'+l+l+')'
    _obs = Observable(_obs_name)
    _process_tex = r"D_0\to "+_tex[l]+r"^+"+_tex[l]+r"^-"
    _obs.set_description(r"Mass-eigenstate rate asymmetry in $" + _process_tex + r"$.")
    _obs.tex = r"$A_{\Delta\Gamma}(" + _process_tex + r")$"
    _obs.add_taxonomy(_process_taxonomy + _process_tex + r"$")
    Prediction(_obs_name, ADeltaGamma_func('D0', l))
